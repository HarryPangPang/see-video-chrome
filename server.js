...
// 视频生成：接收 server 转发的参数，打开 Chrome 访问即梦视频生成页
router.post('/api/generate', async (ctx) => {
    const body = ctx.request.body || {};
    const { projectId, creationType, duration, endFrame, frameMode, model, prompt, ratio, startFrame } = body;
    console.log('[Jimeng] Received generate:', { projectId, creationType, model, ratio, duration });
    ...
    try {
        ...
        await setOptions(page, { creationType, duration, endFrame, frameMode, model, prompt, ratio, startFrame });
