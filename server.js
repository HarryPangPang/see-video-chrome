...
router.post('/api/generate', async (ctx) => {
    ...
    const { projectId, creationType, duration, endFrame, frameMode, model, prompt, ratio, startFrame, imagesDir } = body;
    console.log('[Jimeng] Received generate:', { projectId, creationType, model, ratio, duration, imagesDir: imagesDir ? 'yes' : 'no' });
    ...
    try {
        ...
        await setOptions(page, { creationType, duration, endFrame, frameMode, model, prompt, ratio, startFrame, imagesDir });
